...
# GitHub token for docker compose / containers, sourced from gh's keychain
# (no plaintext; rotate via `gh auth login`). Loud on failure on purpose —
# a silent empty token cost an afternoon of container debugging once.
export GH_TOKEN="$(gh auth token)" || echo "zshrc: gh auth token failed"
