" Base / Misc
  " App Models BAse
  nnoremap <silent> ,amba :call CreateBaseFile(1, 0, 1)<return>/class<return>wviwy/ChangeTopLevelDocumentation<return>viwpa model<esc><down>A < ApplicationRecord<esc>/inner_followup<esc>ddO# Class Methods<return><backspace><backspace><return># Contants<return><backspace><backspace><return># Callbacks<return><backspace><backspace><return># Validations<return><backspace><backspace><return># Attributes<return><backspace><backspace><return># Associations<esc>o<esc>x0Do# Scopes<return>DeleteThis - create default order (if necessary). User LOWER if it should be case insensitive. Otherwise, use regulare order method.<return><backspace><backspace>scope :order_by_ChangeDefault, -> { order('LOWER(ChangeAttribute)') }<return>scope :order_by_ChangeDefault, -> { order(:ChangeAttribute) }<esc>o<esc>x0Do# Services<esc>o<esc>x0Do# Instance Methods<esc>gg/DeleteThis\\|ChangeDefault\\|ChangeAttribute<return>
