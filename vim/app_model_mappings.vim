...
" Validations
  ...
  function! RubyModlesValidatesOptions()
    execute "normal! a,\<return># DeleteThis - if presence is validated elsewhere, make sure to allow_nil OR allow_blank so this isn't validating presence or erroring\<space>\<backspace>\<return>\<backspace>\<backspace>allow_nil: true,\<return>allow_blank: true,\<return>if: ChangeMethodOrProcOrArray,\<return>unless: ChangeMethodOrProcOrArray"
  endfunction
  ...
  nnoremap <silent> ,amvu avalidates :ChangeAttribute, uniqueness: {<return>case_sensitive: false,<return>scope: %i[ChangeAttributes],<return>message: 'the ChangeAttribute "%<value>s" is already taken. Please choose a different ChangeAttribute'<return><space><backspace><esc>:call AppModelsValidatesMessage()<return>a}<esc>:call RubyModlesValidatesOptions()<return>:call AppModelsValidatesSearch()<return>
